...
function [y, num, den, N] = applyIIR(wn, bandType, gain, signalY)
 ...
 % Calls filter functions for all bands and forms composite signal 
function applyEqualizer(handles)
    ...
    for i=1:length(bandpasses)
        ...
        if (get(handles.firRadio, 'Value'))
            ...
            set(handles.orderLabel, 'String', 'Order: 50');
        else
            [y, num, den, N] = applyIIR(wn, bandType, currentGain, handles.y);
            set(handles.orderLabel, 'String', strcat('Order: ', N));
        end
        if (i == selectedBand)
...
% Sets output sample frequency as value taken from user    
